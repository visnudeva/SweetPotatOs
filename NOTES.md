...
- Wallpaper preference is `~/.config/swirl/wallpaper.conf`. `ensure-wallpaper.sh` must **not** overwrite a saved path with the default fallback when the file is briefly missing. Default wallpaper is `DefaultBindsBG.png`; also ships `SweetNeonBG.png`, `SPixelBG.png`. `wallpaper.sh` writes conf **before** calling swaymsg so the preference is never lost on a non-zero swaymsg exit.
