...
- Display layout is managed by **nwg-displays** (GUI) + saved **`~/.config/sway/outputs`**. `Mod+Shift+d` opens nwg-displays (must click **Apply & Save**). After **wlr-randr** terminal changes, press **`Mod+Ctrl+d`** to persist. The swirl wrapper creates empty `outputs`/`workspaces` files before Swirl parses config. Swirl **`include`s** those files; **kanshi** profiles are **generated** from `outputs` (no `mode preferred` fallback — that was resetting custom resolutions on boot). **Do not** ship `~/.config/sway/` in skel — runtime + user saves only.
