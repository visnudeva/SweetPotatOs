...
- Display layout is managed by **nwg-displays** (GUI) + **kanshi** (daemon). `Mod+Shift+d` or the app launcher runs `~/.config/swirl/scripts/nwg-displays.sh`, which finds the Swirl IPC socket (`SWAYSOCK`), creates `~/.config/sway/` if needed, then launches nwg-displays. nwg-displays saves to `~/.config/sway/outputs`; Swirl config includes that file. kanshi auto-applies `~/.config/kanshi/config` profiles on startup. `wlr-randr` shows current output names/modes. **Do not** ship `~/.config/sway/` in skel — the wrapper creates it at runtime.
