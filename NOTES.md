...
- Display layout is managed by **nwg-displays** (GUI) + saved **`~/.config/sway/outputs`**. App launcher entry is **Displays** (`GenericName=nwg-displays`, search `nwg`) — same binary as terminal `nwg-displays`. Desktop `Exec` must use `%h/.config/swirl/scripts/nwg-displays.sh` (not `bash -lc '~…'`). `Mod+Shift+d` opens the same wrapper (Apply & Save in the GUI). After **wlr-randr**, press **`Mod+Ctrl+d`**. The swirl wrapper creates empty `outputs`/`workspaces` before parse; kanshi profiles are generated from `outputs` (no `mode preferred` reset). **Do not** ship `~/.config/sway/` in skel.
