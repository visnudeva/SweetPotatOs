...
check_grep 'click_events' "${ISO}/etc/skel/.config/swirl/scripts/status.sh" "status bar supports click events"
check_grep 'name:"wifi"' "${ISO}/etc/skel/.config/swirl/scripts/status.sh" "status bar has clickable wifi block"
check_grep 'networkmanager_dmenu' "${ISO}/etc/skel/.config/swirl/scripts/status.sh" "Wi‑Fi bar click opens networkmanager-dmenu"
check_nogrep 'network-tray\.sh' "${ISO}/etc/skel/.config/swirl/config" "no Wi‑Fi tray wrapper (use bar click)"
