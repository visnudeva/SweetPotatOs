...
check_grep 'exec nm-applet --indicator' "${ISO}/etc/skel/.config/swirl/config" "Wi‑Fi tray uses nm-applet"
check_nogrep 'network-tray\.sh' "${ISO}/etc/skel/.config/swirl/config" "no custom Wi‑Fi tray wrapper"
check_nogrep 'click_events' "${ISO}/etc/skel/.config/swirl/scripts/status.sh" "status bar is plain text (no click protocol)"
check_nogrep 'network-applet\.py' "${ISO}/etc/skel/.config/swirl/config" "no custom network-applet"
