...
for arg in "$@"; do
  case "${arg}" in
    ...
    --build-sweetpotatos) DO_SWEETPOTATOS=1; DO_ISO=0 ;;
    --build-aur-apps) DO_AUR_APPS=1; DO_ISO=0 ;;
    --build-packages) DO_CALAMARES=1; DO_SWIRL=1; DO_SWEETPOTATOS=1; DO_AUR_APPS=1; DO_ISO=0 ;;
    --help|-h)
      cat <<EOF
Usage: sudo ./build.sh [options]
  ...
  --build-aur-apps    Build yay-bin + shelly-bin + localsend-bin + spore + tuber + swaylock-effects + brave-origin-bin into repo/ only
...
if (( DO_ISO )); then
  ...
  need_repo_pkg sweetpotatos || DO_SWEETPOTATOS=1
  ...
  need_repo_pkg tuber || DO_AUR_APPS=1
  need_repo_pkg swaylock-effects || DO_AUR_APPS=1
...
if (( DO_SWEETPOTATOS )) && ! need_repo_pkg sweetpotatos; then
  build_local_pkg sweetpotatos "${ROOT}/packaging/sweetpotatos"
elif (( DO_SWEETPOTATOS )); then
  echo "[*] sweetpotatos already in ${REPO}/ — skipping"
fi
...
if (( DO_AUR_APPS )); then
  ...
  need_repo_pkg tuber || build_local_pkg tuber "${ROOT}/packaging/tuber"
  # Local PKGBUILD: jirutka fork with clock; replaces Arch swaylock
  need_repo_pkg swaylock-effects || build_local_pkg swaylock-effects "${ROOT}/packaging/swaylock-effects"
...
if ! need_repo_pkg calamares || ! need_repo_pkg swirl || ! need_repo_pkg sweetpotatos \
  ...
  || ! need_repo_pkg localsend-bin || ! need_repo_pkg spore || ! need_repo_pkg tuber \
  || ! need_repo_pkg swaylock-effects \
...
Build them first:
  ...
  sudo ./build.sh --build-packages   # builds calamares, swirl, sweetpotatos, yay-bin, shelly-bin, localsend-bin, spore, tuber, swaylock-effects, brave-origin-bin, waypaper
...
ISO_FINAL_NAME="${SWEETPOTATOS_ISO_NAME:-Sweetpotatos_2026.10_Third_Harvest.iso}"
if [[ -n "${ISO_FINAL_NAME}" ]]; then
  latest_iso="$(find "${OUT}" -maxdepth 1 -name '*.iso' -type f -printf '%T@ %p\n' 2>/dev/null \
    | sort -rn | head -1 | cut -d' ' -f2-)"
  if [[ -n "${latest_iso}" && "$(basename "${latest_iso}")" != "${ISO_FINAL_NAME}" ]]; then
    echo "[*] Renaming $(basename "${latest_iso}") → ${ISO_FINAL_NAME}"
    mv -f "${latest_iso}" "${OUT}/${ISO_FINAL_NAME}"
  fi
fi
